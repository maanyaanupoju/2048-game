...
function addRandomTile() {
    ...
    tile.dom.style.top = `calc(var(--gap-size) + ${r} * (var(--cell-size) + var(--gap-size)))`;
    tile.dom.style.left = `calc(var(--gap-size) + ${c} * (var(--cell-size) + var(--gap-size)))`;
...
function updateDOMPositions() {
    allTiles.forEach(tile => {
        tile.dom.style.top = `calc(var(--gap-size) + ${tile.r} * (var(--cell-size) + var(--gap-size)))`;
        tile.dom.style.left = `calc(var(--gap-size) + ${tile.c} * (var(--cell-size) + var(--gap-size)))`;
...
function undo() {
    ...
    for (let r = 0; r < 4; r++) {
        for (let c = 0; c < 4; c++) {
            let value = previousState.board[r][c];
            if (value) {
                let tile = {
                    r, c, value,
                    dom: document.createElement('div')
...
function move(direction) {
    ...
    let prevState = {
        board: board.map(row => row.map(tile => tile ? tile.value : null)),
        score: score
    };
...
document.addEventListener('touchend', e => {
    if(e.target.closest('#restart-btn') || e.target.closest('#new-game-btn') || e.target.closest('#undo-btn')) return;
